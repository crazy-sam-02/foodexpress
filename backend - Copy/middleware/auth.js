...
try {
  if (!admin.apps.length) {
    ...
    console.log("✅ Firebase Admin SDK initialized using serviceAccount JSON");
  } else {
    firebaseInitialized = true;
...
router.post("/google-login", async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      await user.save();
      console.log("✅ Existing user updated with googleId:", user._id);
    }
...
// Check auth status
router.post("/check-auth", authenticateUser, (req, res) => {
  res.status(200).json({
